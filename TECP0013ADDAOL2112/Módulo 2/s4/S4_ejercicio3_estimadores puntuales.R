...
#con distribución exponencial y parámetro lambda = 3
...
# Obtenemos 1200 muestras aleatorias de tamaño 350 
#y las 1200 medias correspondientes a las muestras

set.seed(65) # Para reproducir las muestras en el futuro
(m1200.350 <- sapply(X = rep(350, 1200), 
                     FUN = rexp, rate = 3))
...
# Ensayo Bernoulli. Con las siguientes instrucciones 
#obtenemos un solo valor, en donde el 0 (fracaso) 
#puede ocurrir con probabilidad 0.45 y el 1 (éxito) 
#con probabilidad p = 0.55

set.seed(345)
sample(x = c(0, 1), size = 1, prob = c(0.45, 0.55))
rbinom(n = 1, size = 1, prob = 0.55)
# Obtenemos 1000 muestras de tamaño 31 de 
#una v.a. Bernoulli con p = 0.55

set.seed(5434) # Para reproducir las muestras en el futuro
m1000.31 <- sapply(X = rep(31, 1000), 
                   FUN = function(n) sample(x = c(0, 1), 
                                            size = n, 
                                            replace = TRUE, 
                                            prob = c(0.45, 0.55)))
...
tail(mdf)
# Observamos que el histograma de las medias 
#es parecida a una campana
...
sd(media1000.31); 
sqrt(0.55*0.45)/sqrt(31) # DE de las 1000 medias y DE de la población de la cual provienen las 1000 muestras dividida por la raíz del tamaño de la muestra


# Obtenemos 1150 muestras aleatorias de tamaño 
#n1 = 54 de una distribución exponencial con 
#parámetro lambda1 = 3.2 y otras 
#1150 muestras aleatorias de tamaño n2 = 41 
#de una distribución exponencial con parámetro 
#lambda2 = 1.5.
...
ggplot(dmdf, aes(dif.medias)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 16)) 
mean(dif.medias);
1/3.2-1/1.5 # Media de las 1150 diferencias de medias y diferencia de medias de las poblaciones de las cuales provienen las 11500 muestras

sd(dif.medias); 
sqrt((1/3.2^2)/54 + (1/1.5^2)/41) #es 1/el parametro lamba
# DE de las 1150 diferencias de medias y 
#DE dada en literatura

# Obtenemos 1100 muestras de tamaño 
#n1 = 48 de una v.a. Bernoulli con 
#p1 = 0.65 y otras 1100 muestras de tamaño 
#n2 = 35 de una v.a. Bernoulli con p2 = 0.53
...
ggplot(dmdf, aes(dif.medias)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 16)) 
mean(dif.medias); 
0.65 - 0.53 # Media de las 1100 diferencias de medias y diferencia de medias de las poblaciones de las cuales provienen las muestras
sd(dif.medias); 
sqrt((0.65*0.35)/48 + (0.53*0.47)/35) # DE de las 1100 diferencias de medias y DE dada en literatura
