# Al inicio es posible que no comprendas todo el código, 
#trata de leerlo e ir asimilando que es lo que realiza cada línea. 

library(ggplot2)
install.packages("scales")
library(scales) 
# Para mejorar la lectura de las etiquetas  en el eje de las x de tipo fecha
...
# A continuación, te presentamos un panorama 
#de la situación que se ha estado viviendo en México, 
#debido al coronavirus. Es información simple, 
#que puede resultar valiosa para algunas personas. 
...
p <- ggplot(mex, aes(x=Fecha, y=NI)) + 
  ...
  labs(x = "Fecha", 
       y = "Incidencia (Número de casos nuevos)",
...
p <- ggplot(mexm, aes(x=Fecha, y=Muertos)) + 
  geom_line( color="red") + 
  geom_point() +
  labs(x = "Fecha", 
...
p <- ggplot(mexm, aes(x=Fecha, y=NM)) + 
  ...
  theme(axis.text.x = element_text(face = "bold", 
        ...
        axis.text.y = element_text(face = "bold", 
                                   color="#993333" , 
                                   size = 10, 
                                   angle = 45, hjust = 1))  # color, Ángulo y estilo de las abcisas y ordenadas
...
p <- p +
  theme(plot.margin=margin(10,10,20,10), 
        plot.caption=element_text(hjust=1.05, size=10)) +
  annotate("text", 
           x = mexm$Fecha[round(length(mexm$Fecha)*0.2)], 
           y = max(mexm$Letalidad)-1, 
           colour = "red", size = 4, 
           label = paste("Última actualización: ", 
...
p <- ggplot(mex, aes(x=Fecha, y=FCI)) + 
  geom_line(color="blue") + 
  labs(x = "Fecha", 
...
p <- p +
  annotate("text", 
           x = mex$Fecha[round(length(mex$Fecha)*0.4)], 
           y = max(mex$FCM), 
           colour = "blue", size = 5, 
           label = paste("Última actualización para infectados: ", 
